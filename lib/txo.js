...
const TxIn = Transaction.Input;
const TxOut = Transaction.Output;
const VarInt = encoding.Varint;
...
const TXO = {
  ...
  inputFromTx(src, index) {
    ...
    const input = {
      i: index,
      e: {
        h: src.prevTxId.reverse().toString('hex'),
        i: src.outputIndex,
        a: address
      },
  ...
  toTx({ src, format }) {
    ...
    return Tx({
      versionBytesNum: 1,
      txIns,
      txInsVi: VarInt(txIns.length),
      txOuts,
      txOutsVi: VarInt(txOuts.length),
      nLockTime: src.lock
    })
  },
  ...
  toTxIn(src) {
    ...
    return TxIn.fromObject({
      ...
      nSequence: src.seq,
      script: script,
      scriptVi: VarInt(script.toBuffer().length)
    })
  },
...
function fromScriptChunk(obj, chunk, index) {
  ...
  // Handle opcode
    ...
    obj[`o${index}`] = OpCode.reverseMap[chunk.opcodenum]
  }
...
function toTxScript(src) {
  return [...Array(src.len).keys()]
    .reduce((script, index) => {
      if (src[`o${index}`])       { script._addOpcode(OpCode[src[`o${index}`]]) }
      else if (src[`b${index}`])  { script._addBuffer(Buffer.from(src[`b${index}`], 'base64')) }
      else if (src[`h${index}`])  { script._addBuffer(Buffer.from(src[`h${index}`], 'hex')) }
      return script
    }, new Script())
}
