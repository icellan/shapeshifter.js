...
const BOB = {
  ...
  toTxIn(src) {
    ...
    return TxIn.fromObject({
      prevTxId: Buffer.from(src.e.h, 'hex').reverse(),
      txOutNum: src.e.i,
      nSequence: src.seq,
      script: script,
...
function fromScriptChunk({tape, t}, chunk, index) {
  ...
  // Handle opcode
    ...
    tape[n].cell.push({
      op: chunk.opcodenum,
      ops: OpCode.reverseMap[chunk.opcodenum],
      i: index - t,
      ii: index
    })
...
function toTxScript({ script, last }, { cell, i }) {
  ...
  cell.forEach(c => {
    if (c.ops)    { script._addOpcode(c.op) }
    else if (c.b) { script._addBuffer(Buffer.from(c.b, 'base64')) }
    else if (c.h) { script._addBuffer(Buffer.from(c.h, 'hex')) }
  })

  if (i < last && cell[n].op !== OpCode.OP_RETURN) {
    script._addBuffer(Buffer.from('|'))
  }
